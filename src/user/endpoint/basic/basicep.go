...
import (
	...
	"github.com/tim5wang/selfman/src/user/server/basic"
)
...
// 传入 Server/basic.go 定义的相关业务接口
// 返回 go-kit/endpoint.Endpoint (实际上就是一个函数签名)
func MakeServerEndPointHello(s basic.IServer) endpoint.Endpoint {
	...
	return func(ctx context.Context, request interface{}) (response interface{}, err error) {
		...
		// 这里实际上就是调用我们在Server/basic.go中定义的业务逻辑
		// 我们拿到了 Request.Name 那么我们就可以调用我们的业务 basic.IServer 中的方法来处理这个数据并返回
		// 具体的业务逻辑具体定义....
...
// 传入 Server/basic.go 定义的相关业务接口
// 返回 go-kit/endpoint.Endpoint (实际上就是一个函数签名)
func MakeServerEndPointBye(s basic.IServer) endpoint.Endpoint {
	// 这里使用闭包,可以在这里做一些中间件业务的处理
	return func(ctx context.Context, request interface{}) (response interface{}, err error) {
		// request 是在对应请求来时传入的参数(这里的request 实际上是等下我们要将的Transport中一个decode函数中处理获得的参数)
		// 这里进行以下断言
